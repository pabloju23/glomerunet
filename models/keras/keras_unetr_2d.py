...
def build_unetr_2d(cf):
    ...
    inputs = L.Input(input_shape) ## (None, 256, 256, 3)
    ...
    patches = L.Reshape((cf["num_patches"], cf["patch_size"]*cf["patch_size"]*cf["num_channels"]))(patches) ## (None, 256, 3072)

    # Positional Embeddings
    patch_embed = L.Dense(cf["hidden_dim"])(patches) ## (None, 256, 768)
    # positions = tf.range(cf["num_patches"])
    # pos_embed = L.Embedding(cf["num_patches"], cf["hidden_dim"])(positions)
